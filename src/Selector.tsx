...
export function Selector() {
  ...
  const [selectedValue, setSeleccionado] = useState<string | null>("Challenges");
  ...
  }, [selectedChallenges, selectedLocations, hasAward, orderBy, isHero]); // Added isHero dependency
    ...
    <div className="flex flex-col min-h-screen">
      ...
      <div className="flex-1 max-w-7xl mx-auto w-full p-6">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2 custom-scrollbar">
             <button 
                onClick={() => setSeleccionado(selectedValue === "Challenges" ? "Locations" : "Challenges")}
                className="whitespace-nowrap px-4 py-2 rounded-lg bg-gray-800 text-white text-xs font-bold uppercase tracking-widest border border-gray-700 hover:bg-gray-700"
             >
                 {selectedValue === "Challenges" ? "Show Locations 🌍" : "Show Challenges ⚡"}
             </button>
             {opciones.find(op => op.nombre === selectedValue)?.subCategorias?.map((sub, i) => {
                const isSelected = selectedValue === "Challenges" 
                  ? selectedChallenges.includes(sub as string)
                  : selectedLocations.includes(sub as string);
                
                return (
                  <button key={i} onClick={() => toggleSelection(sub as string)}
              ...
              return (
                ...
                >
                    <div className="flex justify-between items-start mb-4">
                         ...
                         {hasAward && (
                             <span className="bg-yellow-500/10 text-yellow-500 border border-yellow-500/20 text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded">
                                 {result.badges?.split(',')[0]}
                             </span>
                         )}
                    </div>
